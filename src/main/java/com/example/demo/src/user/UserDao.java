...
public class UserDao {
    ...
    public int createUser(PostUserReq postUserReq) {
        String createUserQuery = "insert into User (email, password, phone) VALUES (?,?,?)"; // 실행될 동적 쿼리문
        Object[] createUserParams = new Object[]{postUserReq.getEmail(), postUserReq.getPassword(), postUserReq.getPhone()}; // 동적 쿼리의 ?부분에 주입될 값
    ...
    public User getPwd(PostLoginReq postLoginReq) {
        String getPwdQuery = "select userIdx, email, password, phone from User where email = ?"; // 해당 email을 만족하는 User의 정보들을 조회한다.
    ...
    // 해당 userIdx를 갖는 유저조회
    public GetMembershipIncludeRes getUser(int userIdx) {
        String getUserQuery = "select User.userIdx, User.email, User.password, User.phone, Membership.membershipName, Membership.nextPayment\n" +
                "from User, Membership\n" +
                "where User.userIdx = Membership.userId and User.userIdx = ?"; // 해당 userIdx를 만족하는 유저를 조회하는 쿼리문
        int getUserParams = userIdx;
        return this.jdbcTemplate.queryForObject(getUserQuery,
                (rs, rowNum) -> new GetMembershipIncludeRes(
                        rs.getInt("userIdx"),
                        rs.getString("email"),
                        rs.getString("password"),
                        rs.getString("phone"),
                        rs.getString("membershipName"),
                        rs.getString("nextPayment")
                        ), // RowMapper(위의 링크 참조): 원하는 결과값 형태로 받기
                getUserParams); // 한 개의 회원정보를 얻기 위한 jdbcTemplate 함수(Query, 객체 매핑 정보, Params)의 결과 반환
    }
    // 해당 userIdx를 갖는 프로필조회
    public List<GetUserProfileStartRes> getUserProfileStart(int userIdx) {
        String getUserQuery = "select Profile.profileIdx\n" +
                "from User, Profile\n" +
                "where User.userIdx = Profile.userId and User.userIdx =?"; // 해당 userIdx를 만족하는 유저를 조회하는 쿼리문
        int getUserParams = userIdx;
        return this.jdbcTemplate.query(getUserQuery,
                (rs, rowNum) -> new GetUserProfileStartRes(
    ...
    // 회원정보 변경
//    public int modifyUserPayment(PatchUserReq patchUserReq) {
//        String modifyUserNameQuery = "update Membership set paymentType = ? where userId = ? "; // 해당 userId를 만족하는 User를 해당 paymemtType으로 변경한다.
//        Object[] modifyUserNameParams = new Object[]{patchUserReq.getMembershipName(), patchUserReq.getUserIdx()}; // 주입될 값들(nickname, userIdx) 순
//
